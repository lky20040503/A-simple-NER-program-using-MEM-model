...
class MEMM():
    ...
    def features(self, words, previous_label, position):
        ...
        if current_word.istitle():
            features["Titled"] = 1
        if current_word.islower():
            features["Lowered"] = 1
    
        ## instant name compare
        #if current_word in self.nltk_names:
        #    features['in_nltk_names'] = 1 

        ##word shape
        if re.search(r'\d',current_word):
            features["no_number"] = 0
        else:
            features["no_number"] = 1 
        if len(current_word) <= 2:
            features["not_satisfy_name"] = 1
...
def predict_sentence(sentence, classifier, MEMM, self=None):
    ...
    for i in range(len(words)):
        ...
        features = MEMM.features(self, words, pre_label, i)
